...
async function countAllStudents() {
    ...
    try {
        ...
        for (let i=1; i<=6; i++) {
            ...
            studentsByClass.forEach(student => allStdsClasses.push(student))
        }
        loader.style.display = '';
        console.log(`Total Number of students: ${allStdsClasses.length}`)
...
async function countAllTeachers() {
    ...
    try {
        ...
        loader.style.display = '';
        console.log(`Total Number of Teachers: ${allTeachersArray.length}`)
...
stdNavGradesSelectExam.addEventListener('change', () => {
    gradesTerm = stdNavGradesSelectExam.value;
    readGradesFunc()
})
...
function displayAdminStdMarksFunc() {
    ...
    adminstdMarksArr.forEach((student,i) => {
        ...
        // let s = `.${loggedInUserInfo[0].dets.proffession.asigned_subjs[0]}1`
        // console.log(s)
        // let subj_1 = student.marks+''+s
        td += `
...
async function EditAdminStdMarksFunc(i) {
    ...
    try {
        loader.style.display = 'flex';
        const res = await fetch(`${url}/updatedata/marks_${adminStdMarksYear-1}/${adminStdMarksClass}/${adminStdMarksTerm}/${adminStdMarksExam}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'Application/json'
            },
...
async function deleteTeacherDataBtn(index) {
...
async function logiInTeacherFunc() {
    ...
    try {
        ...
        if (loggedInUserInfo.length >= 1) {
            if (loggedInUserInfo[0].dets.system.role == "library" || loggedInUserInfo[0].dets.system.role == "admin") {
                setTimeout(() => {
            ...
            setTimeout(()=> {
                ...
                // alert('Hello Tr. '+loggedInUserInfo[0].dets.fname + ' '+loggedInUserInfo[0].dets.lname)
            ...
            main.style.display = 'block'
                ...
                alert('Yure not a Librarian or an Admin, youre a:  '+loggedInUserInfo[0].dets.system.role)
            }
